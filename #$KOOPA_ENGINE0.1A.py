...
# -*- coding: utf-8 -*-
# ======================================================================
#  KOOPA ENGINE 0.2.B  —  SM64 PC PORT AUTHENTIC
#  Super Mario 64 · Pygame Edition
#  Copyright (C) 1999-2026  A.C Holdings / Team Flames
#  Entry point: python3 KoopaEngine02.py
#
#  Requires: Python 3.14+, pygame
#  python3 -c "import sys; assert sys.version_info>=(3,14),'Need 3.14+'"
# ======================================================================
...
if sys.version_info < (3, 14):
    print(f"[KoopaEngine 0.2] WARNING: Python {sys.version} detected. 3.14+ recommended.")
...
# Movement — SM64 Famicom-accurate analog constants
MOVE_ACCEL  = 1.8
MOVE_DECEL  = 0.85
MAX_SPEED   = 14
SPRINT_MULT = 1.6
...
#  Sourced from sm64ex / sm64-port decomp palette data
# ======================================================================
...
#  Matches sm64ex / sm64-port sky colours exactly
# ======================================================================
SM64_SKIES = {
    # (sky_top, sky_bottom, fog_color)
...
# ======================================================================
#  3D ENGINE  (KoopaEngine 0.2 core — unchanged from 0.1)
# ======================================================================
class Vector3:
    __slots__ = ['x','y','z']
...
# ======================================================================
#  MARIO — SM64-accurate character capsule (first-person, no visible model)
# ======================================================================
class Mario(Mesh):
    def __init__(self, x, y, z):
...
# ======================================================================
class Star(Mesh):
    """SM64 Power Star — gold cross-puff shape"""
...
def build_castle_grounds():
    ...
    m.add_cube(80,96,5,   0,320,-1008, (240,160,200))  # Peach glass
    for wx in [-160,160]:
...
def build_hazy_maze_cave():
    ...
    m.add_cube(80,36,120,   0,18,0, (0,120,0))    # Dorrie
...
# ======================================================================
#  RENDERER  (KoopaEngine 0.2 — painter's algo + distance fog)
# ======================================================================
...
# ======================================================================
#  MENU HEAD  (SM64 Mario head — red hat, blue overalls, skin face)
# ======================================================================
def create_menu_head():
    m=Mesh()
    m.add_cube(40,36,40,  0, 0, 0, SKIN)          # face
    m.add_cube(44,12,44,  0,20, 0, MARIO_RED)      # hat brim top
    m.add_cube(52, 4,52,  0,15,10, MARIO_RED)      # hat brim
    m.add_cube(10,10,10,  0,-2,22, SKIN)           # nose
    m.add_cube(24, 6, 4,  0,-10,21, MUSTACHE_BLACK)# mustache
    m.add_cube(10,12, 2, -12,6,20, WHITE)          # eye white L
    m.add_cube(10,12, 2,  12,6,20, WHITE)          # eye white R
    m.add_cube( 4, 6, 3, -12,6,21, EYE_BLUE)      # pupil L
    m.add_cube( 4, 6, 3,  12,6,21, EYE_BLUE)      # pupil R
    m.add_cube(42,24,10,  0, 0,-18, MARIO_BLUE)   # overalls back
    return m
...
def main():
    ...
    pygame.display.set_caption("SUPER MARIO 64 — KOOPA ENGINE 0.2  |  A.C Holdings / Team Flames")
    ...
    menu_head: Mesh = create_menu_head()
    ...
    mario: Optional[Mario] = None
    current_level_mesh: Optional[Mesh] = None
    ...
    collected_stars: set[str] = set(); total_coins=0
    ...
    def draw_sky(level_id):
        sky=SM64_SKIES.get(level_id,((56,100,184),(192,220,255),(172,204,248)))
        top,bot,fog=sky
    ...
    def draw_hud():
        ...
        screen.blit(font_hud.render(f"● {total_coins}",True,cc),(96,HEIGHT-38))
        lives = mario.lives if mario is not None else 4
        health = mario.health if mario is not None else 8
        screen.blit(font_hud.render(f"♥ x{lives}",True,RED),(180,HEIGHT-38))
        for i in range(8):
            c=RED if i<health else DARK_GREY
            pygame.draw.rect(screen,c,(268+i*18,HEIGHT-38,14,14))
        screen.blit(font_hud.render(f"FPS:{int(clock.get_fps())}",True,METAL_GREY),(WIDTH-80,HEIGHT-38))
        if current_level_id:
    ...
    while running:
        ...
        for event in pygame.event.get():
            ...
            if current_state==STATE_MENU:
                ...
                if event.type==pygame.KEYDOWN:
                    ...
                    elif event.key==pygame.K_e and mario is not None:
        ...
        if current_state==STATE_MENU:
            # SM64 title screen — navy-to-black gradient
            for y in range(HEIGHT):
                t=y/HEIGHT
            ...
            # Drop shadow then title
            ts=font_title.render("AC HOLDINGS KOOPA ENGINE",True,WHITE)
            th=font_title.render("AC HOLDINGS KOOPA ENGINE",True,MARIO_RED)
            screen.blit(th,(WIDTH//2-ts.get_width()//2+3,ly+3))
            screen.blit(ts,(WIDTH//2-ts.get_width()//2,ly))
            sub=font_small.render("Credit to Nintendo of Japan  ·  A.C Holdings / Team Flames 1999-2026",True,STAR_GOLD)
            screen.blit(sub,(WIDTH//2-sub.get_width()//2,ly+55))
            ver=font_small.render("All 26 Levels  ·  60 FPS  ·  First-Person Lakitu",True,LIGHT_GREY)
            ...
            for i,item in enumerate(menu_items):
                ...
                if i==selected_index:
                    pygame.draw.polygon(screen,MARIO_RED,[(32,my+i*38+8),(44,my+i*38+14),(32,my+i*38+20)])
            sc=font_small.render(f"Stars: {len(collected_stars)}/{STAR_TOTAL}",True,STAR_GOLD)
            ...
            if active_overlay=="how":
                ...
                lines=["CONTROLS:","Mouse — Look Around","WASD / Arrow Keys — Move  (Shift = Sprint)",
                       "SPACE — Jump","E — Enter Door / Painting / Exit Level",
                       "TAB — Toggle Mouse Capture","ESC — Pause","",
                       "FIRST-PERSON LAKITU CAMERA","Explore Peach's Castle and all 15 courses!",
                       "Collect Stars to unlock new areas.","","ENTER to close"]
                ...
                lines=["KOOPA ENGINE 0.2  —  SUPER MARIO 64 PC PORT","",
                       "Engine: Pure Pygame 3D — KoopaEngine 0.2",
                       "All 15 Main Courses + 5 Secret + 3 Bowser Levels",
                       "SM64 PC-port-accurate vertex colour palettes",
                       "Authentic blue-sky gradients + distance fog","",
                       "Copyright (C) 1999-2026  A.C Holdings / Team Flames",
                       "Inspired by: Super Mario 64 (Nintendo, 1996)","","ENTER to close"]
                for i,ln in enumerate(lines):
                    c=STAR_GOLD if i==0 else WHITE
                    f=font_menu if i==0 else font_small
            ...
            for i in range(vs,ve):
                ...
                y+=27
            screen.blit(font_small.render("UP/DOWN: Navigate  |  ENTER: Play  |  ESC: Back",
                        True,METAL_GREY),(WIDTH//2-180,HEIGHT-28))

        elif current_state==STATE_GAME:
            fog_color=draw_sky(current_level_id or "castle_grounds")
            ...
            if not mouse_captured:
                if keys[pygame.K_UP]:    accel_x+=fwd_x*MOVE_ACCEL*sprint;   accel_z+=fwd_z*MOVE_ACCEL*sprint
                if keys[pygame.K_DOWN]:  accel_x-=fwd_x*MOVE_ACCEL*0.6;     accel_z-=fwd_z*MOVE_ACCEL*0.6
                if keys[pygame.K_LEFT]:  accel_x-=right_x*MOVE_ACCEL*0.8;   accel_z-=right_z*MOVE_ACCEL*0.8
                if keys[pygame.K_RIGHT]: accel_x+=right_x*MOVE_ACCEL*0.8;   accel_z+=right_z*MOVE_ACCEL*0.8
            ...
            hint=("E: Enter Door / Painting" if current_level_id and "castle" in current_level_id
                  else "E: Exit Level") if current_level_id else ""
            if hint:
            ...
            screen.blit(font_big.render("PAUSED",True,STAR_GOLD),(WIDTH//2-60,140))
            rows=[("ESC — Resume",WHITE),("R — Restart Level",WHITE),("Q — Quit to Menu",WHITE),("",WHITE),
                  (f"Stars: {len(collected_stars)}/{STAR_TOTAL}",STAR_GOLD),
                  (f"Coins: {total_coins}",COIN_YELLOW),
                  (f"Lives: {mario.lives if mario else 4}",RED)]
            for i,(t,c) in enumerate(rows):
                if t: screen.blit(font_menu.render(t,True,c),(WIDTH//2-120,212+i*34))

        # SM64-style CRT scanlines (subtle authenticity layer)
